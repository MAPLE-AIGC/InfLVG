...
class WanTextEncoder(TextEncoderInterface):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(os.path.join(os.environ["WAN_ROOT"], "models_t5_umt5-xxl-enc-bf16.pth"),
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(os.environ["WAN_ROOT"], "google/umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(VAEInterface):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=os.path.join(os.environ["WAN_ROOT"], "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(DiffusionModelInterface):
    def __init__(self):
        ...
        # self.model = WanModel.from_pretrained("/storage/qiguojunLab/qiguojun/home/Models/Wan-AI/Wan2.1-T2V-1.3B/")
        # self.model.eval()
